...
TORCH_LIBRARY_FRAGMENT(moe_ascend_npu, m)
{
    ...
    m.def(
        "fused_moe_w4a16_cached(Tensor x_in, Tensor w13_weight, Tensor w13_scales, "
        "Tensor w2_weight, Tensor w2_scales, Tensor slot_ids, Tensor topk_weights) -> Tensor");
...
TORCH_LIBRARY_IMPL(moe_ascend_npu, PrivateUse1, m)
{
    ...
    m.impl("fused_moe_w4a16_cached", TORCH_FN(moe_ascend_npu::npu_kernel::fused_moe_w4a16_cached));
