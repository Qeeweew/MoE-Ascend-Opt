...
| `--moe-expert-cache-swap-per-update` | int | `8` | 备用 slot 数和最大替换批量；缓存满载后每轮最多替换 8 个 |
| `--moe-expert-cache-update-interval` | int | `16` | decode 更新基础周期；缓存满载后自动退避 |
| `--moe-expert-cache-warmup-steps` | int | `16` | 开始动态替换前的有效 decode step 数 |
| `--moe-expert-cache-decay` | float | `0.95` | decode route 频率 EMA 衰减系数 |

动态缓存与全层 `--enable-moe-offload` 互斥；动态缓存内部已经包含 Q4_0 CPU 后备路径。K 个活动 slot 在 KV Cache 规划前按层均匀初始化；Prefill 可以读取专家缓存，命中走 NPU、未命中走 CPU，但不参与热度统计或替换。缓存模式保持 NPU Graph 开启，由 replay 外部 hook 发布 decode 阶段的 slot table 变更。
