...
void moe_q8_forward_ptr_impl(
    ...
    int64_t num_tokens, int64_t hidden_dim, int64_t num_experts,
    int64_t intermediate_size, int64_t tp_size, int64_t top_k) {
    moe_forward_ptr_impl<quant::QuantType::Q8_0, T>(
        x_in_ptr, y_out_ptr, routing_weights_ptr, selected_experts_ptr,
        reinterpret_cast<const void* const*>(gate_up_qs_tp),
        gate_up_d_tp,
        reinterpret_cast<const void* const*>(down_proj_qs_tp),
        down_proj_d_tp,
...
template <typename T>
void moe_q4_forward_ptr_impl(
    const T* x_in_ptr,
    T* y_out_ptr,
    const float* routing_weights_ptr,
    const int32_t* selected_experts_ptr,
    const uint32_t* const* gate_up_qs_tp,
    const at::Half* const* gate_up_d_tp,
    const uint32_t* const* down_proj_qs_tp,
    const at::Half* const* down_proj_d_tp,
    int64_t num_tokens, int64_t hidden_dim, int64_t num_experts,
    int64_t intermediate_size, int64_t tp_size, int64_t top_k) {
    moe_forward_ptr_impl<quant::QuantType::Q4_0, T>(
        x_in_ptr, y_out_ptr, routing_weights_ptr, selected_experts_ptr,
        reinterpret_cast<const void* const*>(gate_up_qs_tp),
        gate_up_d_tp,
        reinterpret_cast<const void* const*>(down_proj_qs_tp),
