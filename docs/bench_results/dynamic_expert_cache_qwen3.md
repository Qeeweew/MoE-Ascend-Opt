...
K=128 的 local decode 相对 CPU 约 +2.1%，E2E median 约 +0.5%，基本打平。原因是小缓存只覆盖约 23% expert routes，NPU cached kernel、event join、prefill/HTTP 开销和低频 cache update 波动会抵消局部 decode 收益。
...
3. K=128 的端到端收益仍不足，local decode 约 +2.1%，E2E 约 +0.5%。当前瓶颈不再是“CPU 少算一个 expert 不变快”，而是小 K hit rate 和 cache 控制面开销。
